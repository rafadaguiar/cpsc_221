...
SliderPuzzle::SliderPuzzle(int r, int c, string config) : rows(r), cols(c) {
  ...
  // We assume that the string 'config' is a list of unique integers
...
int SliderPuzzle::getBadness() {
  ...
  for (int i=0; i < rows; i++) {
	...
	// Casts just to avoid aggressive type conversion warnings.
	cost += (int)abs(i-target_row) + (int)abs(j-target_col);
      }
    }
  }
  return cost;
